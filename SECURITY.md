...
**IMPORTANT:** `tachu` is currently in the **1.0.0-alpha** stage. The project is undergoing active development and frequent architectural changes. 

As such:
- The codebase has not yet undergone a formal security audit.
- Security vulnerabilities are likely to exist.
- We **strongly advise against** using this version in production environments or for handling sensitive, mission-critical data.

---

## Supported Versions

During the alpha phase, we only provide security updates for the most recent release.

| Version | Supported          |
| ------- | ------------------ |
| 1.0.0-alpha.x | :white_check_mark: Yes |
| < 1.0.0-alpha | :x: No                |
...
- **Fix:** For valid vulnerabilities, we will prioritize a fix in the next alpha tag or via a direct commit to the `main` branch.
