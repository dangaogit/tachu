...
type StreamChunk =
  ...
  | { type: 'tool-loop-step'; step: number; maxSteps: number; stepId?: string }
  | { type: 'tool-loop-step-end'; step: number; success: boolean; error?: StructuredError }
  | { type: 'tool-call-start'; callId: string; tool: string; argumentsPreview: string }
  | { type: 'tool-call-end'; callId: string; tool: string; success: boolean; error?: StructuredError }
  | { type: 'tool-loop-final'; steps: number; success: boolean }
  | { type: 'done';         output: EngineOutput };

type StreamEnvelope = {
  correlation: { traceId: string; requestId: string; sessionId: string; turnId: string };
  subject?: { tenant?: string; userId?: string };
};

type StructuredError = { code: string; message: string; retryable: boolean };
```
...
| `done` | 流末尾（成功、降级或失败的最终 EngineOutput） | Engine 主循环 finally | UI 解锁输入；`output.metadata.outcome` 决定后续行为 |
...
| 5 | 任务拆分 | 校验通过的意图 | 依赖图 + 方案列表（`tasks.length ≥ 1`） | 否 |
| 6 | 依赖图校验 | Planning router / Plan 模式产出的依赖图 | 校验后的执行计划 | 否 |
| 7 | 子任务执行 | 执行计划 | 各子任务结果（含内置 `direct-answer` Sub-flow 的自然语言答复） | 视子任务 |
| 8 | 结果验证 | 执行结果 | 通过/不通过 + 诊断 | 否；semantic judge 为可选扩展 |
...
| 路径 | 触发条件 | 行为 |
| --- | --- | --- |
| Direct-answer 路由 | `complexity === 'simple'` 或 `complex` 且无可见 Tool | 产出单步 Plan：`{ ref: 'direct-answer', input: { prompt, warn? } }` |
| Tool-use 路由 | `complexity === 'complex'` 且 Registry 存在 ≥1 个可见 Tool | 产出单步 Plan：`{ ref: 'tool-use', input: { prompt, toolNames? } }`；多步工具选择在 Phase 7 Agentic Loop 内完成 |
| Plan 模式 | 上层显式指定 `planMode: true` | 产出同上路由结果后发出 `plan-preview` 事件，由宿主 hook 审阅；当前不引入独立 LLM 预规划器 |
| 模板匹配 | 匹配到预定义 Plan 模板 | 当前未实现，保留为扩展点 |
...
#### PlanningResult 输出（当前主路径为确定性 routing，不由 LLM 预拆分）
...
**当前实现范围**（Tachu v1，`1.0.0` 前；详见 §8.1）：

- Phase 8 仅做**确定性失败扫描**：遍历 `taskResults`，若任意 task 的 `status === 'failed'`，产出 `diagnosis = { type: 'execution_issue', reason, failedTaskIds }`，否则 `passed: true`
- **未实现** LLM 结果诊断：不会从语义层判断输出"质量不达标"；`diagnosis.type` 在当前实现中只取 `execution_issue`
- **未实现**自动重试循环：诊断信号只产出一次，写入 `EngineOutput.steps`；编排控制面 `ExecutionOrchestrator.switchToNextPlan` 可在多方案场景下手动切换，但 Engine 主循环不会自动重走 Phase 7

**后续路线图**（仍为 Tachu v1，非下一代框架）：
...
| 职责 | 说明 |
| --- | --- |
| 规划输出管理 | 接收 Planning router / Plan 模式产出的 `PlanningResult`，传递给依赖图校验 |
| 依赖图校验 | 调用确定性校验逻辑 |
...
| `tool-call-end` | 工具调用结束（成功 / 失败 / 被拒绝） | 带 `callId` / `tool` / `success` / `durationMs` / `parentStepId` / `error?: { code, message, retryable }` |
| `tool-loop-final` | 循环结束 | 带 `steps` / `success` |

所有 `tool-call-end` 在失败时都填结构化 `error`：`TOOL_LOOP_APPROVAL_DENIED` / `TOOL_LOOP_UNKNOWN_TOOL` / `TOOL_LOOP_TOOL_EXECUTION_FAILED` 等错误码写入 `error.code`，同时保留 `error.message` 与 `error.retryable`，供 UI 层精准渲染。

**观测事件（`ObservabilityEmitter`）**：

```
phase_enter     { provider, model, toolCount, maxSteps, parallelism }
tool_loop_step_start { step, maxSteps, stepId, parentStepId, selectedTools, argumentsPreview, retryCount }
tool_loop_step_end   { step, stepId, parentStepId, success, stopReason, failureReason, selectedTools, argumentsPreview, retryCount, error? }
progress        { stage: "approval-pending" | "approval-granted", tool, callId, triggeredBy }
tool_call_start { tool, callId, argumentsPreview }
tool_call_end   { tool, callId, parentStepId, success, durationMs, outputLength?, error? }
...
**工具调用记录汇总**：每次 `executeSingleToolCall` 结束都会向 `onToolCall(record)` push 一条 `ToolCallRecord { callId, tool, parentStepId?, durationMs, success, source: "tool", error? }`；主干 `runStream` 收集后写入 `EngineOutput.metadata.toolCalls`，供业务侧做计费、重放或审计。
...
### 8.1 重试与降级（当前落地范围，Tachu v1）

> **当前实现范围声明**：`1.0.0-rc.0` 已接线 turn 级 retry / Phase 8 `retry|degrade|handoff`。Provider 运行时降级、完整系统级 / 任务级自动重试循环与 ExecutionPolicy 仍留在后续路线图。原因：自动重试与 provider fallback 涉及幂等性、副作用回放、模型路由与预算消耗等正交问题。

#### 预留：`Provider` 运行时降级（v1.x+）
...
**未实现：** `DefaultModelRouter` 不遍历该列表；无 `provider_fallback` 事件 emit；LLM 失败时各 phase 走启发式 / 抛错 / 用户可见错误文案。

**计划：** `1.0.0` 之后的首个 **v1.x** 小版本（或更高）再落地 ExecutionPolicy + observability。

#### 当前已实现：Phase 8 诊断与 turn 级 outcome（部分）
...
注意：当前实现并不会自动循环重试 Phase 7；automatic loop 留在后续路线图。
```

#### 路线图：自动重试循环

下列能力后续由 ExecutionPolicy 承载，**当前不交付，文档不开放配置**：
