...
import type { EngineOutput, ExecutionContext, InputEnvelope } from "@tachu/core";
...
const createRunContext = (sessionId: string, startedAt: number): ExecutionContext => {
...
export const runCommand = defineCommand({
 ...
 // - 第二次：立即以 130 硬退出
    ...
    try {
      const context = createRunContext(sessionId, Date.now());
      ...
      if (args["text-to-image"]) {
        ...
        const t = raw.trim();
        input = buildIncludeToolsInputEnvelope(
