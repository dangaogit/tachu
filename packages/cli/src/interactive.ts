...
import type { Engine, ExecutionContext, MemoryEntry } from "@tachu/core";
 ...
 // 重置 sigint 计数（用户主动输入）
      ...
      if (imageSlash) {
        ...
        try {
          ...
          const context = createTurnContext(sessionHolder.session.id, startTime);
      ...
      if (textToImageIntent) {
        ...
        const input = buildIncludeToolsInputEnvelope(
          prompt,
          [DEFAULT_TEXT_TO_IMAGE_TOOL_NAME],
          "cli-chat",
        );
