...
/**
 * 判断 `tool-call-end` 的结构化错误消息是否来自审批拒绝。
 *
 ...
 *
 * **生命周期**：每次 `finalize()` 结束后清零，保证多轮共享同一渲染器时
 * 不会跨轮累加。否则在 chat 模式下，第 N 轮的 streamedBody 会保留前
 * N-1 轮的正文，导致与本轮 `EngineOutput.content` 不等、dedupe 失败、
 * 答复被 finalize 又打印一次。
 */
 ...
 */
 ...
 // Agentic Loop 每一轮思考开始：非 verbose 模式下用 spinner 提示。
      ...
      case "tool-call-start": {
        ...
        const parent = chunk.parentStepId ? ` parent=${chunk.parentStepId}` : "";
        process.stdout.write(
          colorize(`→ 调用工具 ${chunk.tool} callId=${chunk.callId}${parent}  ${sanitizedPreview}\n`, "yellow"),
        );
        break;
      }
      case "tool-call-end": {
        ...
        if (chunk.success) {
          const parent = chunk.parentStepId ? ` parent=${chunk.parentStepId}` : "";
          process.stdout.write(
            colorize(`  ✓ ${chunk.tool} callId=${chunk.callId}${parent} (${chunk.durationMs}ms)\n`, "green"),
          );
        } else {
          const rawMessage = chunk.error?.message ?? "";
          const sanitized = sanitizeUserText(rawMessage).trim();
          const code = chunk.error?.code ?? "UNKNOWN";
          const retryable = chunk.error?.retryable === true ? "true" : "false";
          const parent = chunk.parentStepId ? ` parent=${chunk.parentStepId}` : "";
          const isDenied =
            chunk.error?.code === "TOOL_LOOP_APPROVAL_DENIED" ||
            isApprovalDenial(sanitized);
          const headline = isDenied
            ? `  ✗ ${chunk.tool} 已拒绝 callId=${chunk.callId}${parent} (${chunk.durationMs}ms)`
            : `  ✗ ${chunk.tool} 执行失败 callId=${chunk.callId}${parent} (${chunk.durationMs}ms)`;
          process.stdout.write(colorize(`${headline}\n`, "red"));
          if (sanitized.length > 0) {
            process.stdout.write(
              colorize(`     原因: ${sanitized} [code=${code} retryable=${retryable}]\n`, "gray"),
            );
          }
        }
        break;
 ...
 // 把 cached 量按 0.5 系数从 billable 里扣除；总 token 数仍按全额展示。
        ...
        process.stdout.write(
          colorize(
            `\n[done:${metadata.outcome}] duration=${metadata.durationMs}ms tokens=${formatTokensK(total)}${cachedTag} cost≈$${cost}\n`,
            "blue",
          ),
        );
        break;
 ...
 */
 ...
 // chat 模式下渲染器跨轮复用：finalize 结束即视为本轮终结，清空累积避免下一轮 dedupe 误判。
    this.streamedBody = "";
  }
