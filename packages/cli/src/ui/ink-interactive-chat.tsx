...
import type { Engine, EngineOutput, ExecutionContext, MemoryEntry } from "@tachu/core";
...
const createTurnContext = (sessionId: string, startedAt: number): ExecutionContext => {
  ...
  const runOneTurn = useCallback(
    async (line: string): Promise<void> => {
      ...
      if (imageSlash) {
        ...
        const context = createTurnContext(sessionHolder.current.session.id, startTime);
      ...
      if (textToImageIntent) {
        ...
        const inputPayload = buildIncludeToolsInputEnvelope(
          prompt,
          [DEFAULT_TEXT_TO_IMAGE_TOOL_NAME],
          "cli-chat",
        );
