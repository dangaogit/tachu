 ...
 */
 ...
 // 深单 loop 脊柱的对外粗粒度里程碑（turnStart / turnStop）。
      ...
      case "tool-call-start": {
        ...
        this.appendLog(`→ 调用工具 ${chunk.tool} callId=${chunk.callId}${parent}  ${sanitizedPreview}`);
        this.push();
        break;
      }
      case "tool-call-end": {
        if (chunk.success) {
          const parent = chunk.parentStepId ? ` parent=${chunk.parentStepId}` : "";
          this.appendLog(`  ✓ ${chunk.tool} callId=${chunk.callId}${parent} (${chunk.durationMs}ms)`);
        } else {
          ...
          if (sanitized.length > 0) {
            this.appendLog(`     原因: ${sanitized} [code=${code} retryable=${retryable}]`);
          }
        }
        this.push();
        break;
      ...
      case "done": {
        ...
        this.state.doneSummary = `[done:${metadata.outcome}] duration=${metadata.durationMs}ms tokens=${formatTokensK(total)} cost≈$${cost}`;
        this.push();
        break;
      }
      default: {
