...
describe("runIntentPhase (Phase 3 — Intent Analysis, pure classification)", () => {
 ...
 test("LLM 返回 turnPolicy subset → 归一化写入 metadata.turnPolicy", async () => {
    const provider = new StubProvider(async () => ({
      content: JSON.stringify({
        complexity: "complex",
        intent: "summarize with charts",
        contextRelevance: "unrelated",
        turnPolicy: {
    ...
    const { env } = buildEnv(provider);
    const out = await runIntentPhase(buildSafetyState("Please analyze the tech market trends with charts"), {
      ...env,
      scope: {
        intentAgentContext: {
          tools: [{ name: "image.qwen", description: "text to image" }],
          skills: [{ name: "chart-output", description: "charts" }],
        },
      },
    });
    expect(out.intent.turnPolicy?.excludeTools).toEqual(["image.qwen"]);
    expect(out.input.metadata?.turnPolicy).toEqual({
      excludeTools: ["image.qwen"],
      includeTools: [],
      explicitSkills: [],
      excludeSkills: [],
 ...
 test("User explicit selections 注入 intent system prompt", () => {
    const prompt = intentInternals.buildIntentSystemPrompt(
      buildConfig(),
      {
        explicitSkillNames: ["chart-output"],
        explicitToolNames: ["image.qwen"],
        intentAgentContext: {
          skills: [{ name: "chart-output", description: "charts" }],
        },
      },
    );
    expect(prompt).toContain("User explicit selections");
    expect(prompt).toContain("skills: chart-output");
    expect(prompt).toContain("tools: image.qwen");
  });

 test("intent.systemPromptBase 替换 base 且仍追加 Agent Context / explicit selections", () => {
    const hostBase = "HOST-INTENT-BASE";
    const prompt = intentInternals.buildIntentSystemPrompt(
      { ...buildConfig(), intent: { systemPromptBase: hostBase } },
      {
        explicitSkillNames: ["chart-output"],
        explicitToolNames: ["image.qwen"],
        intentAgentContext: {
          tools: [{ name: "kb.search", description: "knowledge base" }],
        },
      },
    );
    expect(prompt.startsWith(hostBase)).toBe(true);
    expect(prompt).toContain("### Agent Context");
    expect(prompt).toContain("User explicit selections");
    expect(prompt).toContain("kb.search");
  });

 test("预置 includeTools 信封跳过 Intent LLM", async () => {
    let calls = 0;
    const provider = new StubProvider(async () => {
      calls += 1;
      return {
    ...
    expect(calls).toBe(0);
    expect(out.input.metadata?.turnPolicy?.includeTools).toEqual(["image.qwen"]);
  });
 ...
 test("disableSimpleMarkers=true 时，短 simple 输入不走 fast-path，交由 LLM 分类", async () => {
    let calls = 0;
    const provider = new StubProvider(async () => {
      calls += 1;
      return {
 ...
 // 覆盖 config，加入 disableSimpleMarkers=true
    const config: EngineConfig = { ...buildConfig(), intent: { disableSimpleMarkers: true } };
    const observability = new DefaultObservabilityEmitter();
    const events: EngineEvent[] = [];
    observability.on("*", (e) => events.push(e));
    const env: PhaseEnvironment = {
 ...
 // "列出流程" ≤ 40 字且命中 STRONG_SIMPLE_MARKERS，但 disableSimpleMarkers=true 应阻止 fast-path
    const out = await runIntentPhase(buildSafetyState("列出流程"), env);

    expect(calls).toBe(1); // LLM 被调用
    expect(out.intent.complexity).toBe("complex");
    expect(
      events.some(
        (e) =>
 ...
 test("explicit tool mention fast-path forces complex and skips intent LLM", async () => {
    const provider = new StubProvider(async () => {
      throw new Error("intent LLM should not be called for explicit tool mention");
    });
    const { env, events } = buildEnv(provider);
    env.scope = {
      intentAgentContext: {
        tools: [{ name: "image.qwen", description: "Qwen image generation tool" }],
      },
    };
    const out = await runIntentPhase(buildSafetyState("使用 image.qwen 工具画一只猫"), env);
    expect(out.intent.complexity).toBe("complex");
    expect(out.input.metadata?.turnPolicy?.excludeTools).toEqual([]);
    expect(
      events.some(
        (e) =>
          e.phase === "intent" &&
