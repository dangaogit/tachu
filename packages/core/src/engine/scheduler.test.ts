...
describe("TaskScheduler", () => {
 test("executes tasks in dag order", async () => {
    const scheduler = new TaskScheduler(async (task) => ok(task.id));
    const plan = {
      rank: 1,
      tasks: [
 ...
 test("marks timeout path with structured timeout error", async () => {
 ...
 test("preserves partial output when a timed-out task settles after abort", async () => {
    const partial = {
      kind: "tool-use-result",
      status: "partial",
      observations: [{ source: "tool", tool: "search", callId: "call-1", text: "result" }],
      steps: [],
      error: { code: "TOOL_LOOP_ABORTED", message: "aborted", retryable: false },
    };
    const scheduler = new TaskScheduler(
      async (_task, _context, signal) =>
        new Promise((resolve) => {
          signal.addEventListener("abort", () => resolve(ok(partial)), { once: true });
        }),
    );
    const plan = {
      rank: 1,
      tasks: [{ id: "tool-use", type: "sub-flow" as const, ref: "tool-use", input: {} }],
      edges: [],
    };
    ...
    expect(results[0]?.output).toEqual(partial);
  });

 test("propagates abort signal before scheduling", async () => {
    const scheduler = new TaskScheduler(async () => ok("ok"));
    const plan = {
      rank: 1,
