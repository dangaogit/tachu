...
export interface Message {
 ...
 /**
 * 本条消息**同生命周期**的旁路 Resource Pool。
 *
 * 正文以 `[[ref:<kind>:<key>]]` 占位 token 引用其中条目；物化（Reference
 * Materialization）在 Provider 边界按「正文 token ∩ 本条 resources ∩ 下游需求 ∩
 * 模型能力」展开。匹配只认 `key`，不认 `displayLabel`（防注入）。
 */
  resources?: ResourceReference[] | undefined;
 /**
 * assistant 角色消息中附带的工具调用请求（
 *
