 ...
 *
 * 设计意图（见 detailed-design §9.2）：Engine 自身保持单例 + 默认配置；会话层
 ...
 *   未提供时回退默认值，与现有行为一致。
 * - `additionalRules` / `additionalSkills` / `additionalTools`：与 `registry.list(...)`
 *   **取并集**后传给 PromptAssembler。这样 registry 里跨会话的全局基线（如 safety
 *   rule）始终生效，session 只**叠加**自己的本轮项；不是替换语义。
...
export interface SessionScope {
  ...
  /**
   * Host-provided stable id factory for public stream objects. cube-agentic-engine
   * injects its SFID factory here so reasoning step ids and usage attribution ids
