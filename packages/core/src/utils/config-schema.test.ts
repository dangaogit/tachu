...
describe("validateEngineConfig", () => {
 test("returns defaults when undefined", () => {
    ...
    expect(config.runtime.defaultTaskTimeoutMs).toBe(600_000);
 ...
 test("rejects retired legacy skill activation mode", () => {
    expect(() =>
      validateEngineConfig({
        runtime: { skillActivationMode: "legacy" },
      }),
    ).toThrow(ValidationError);
  });
