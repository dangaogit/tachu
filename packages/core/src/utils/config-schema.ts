...
import { SYSTEM_PROMPT_BASE_MAX_BYTES } from "./system-prompt-base";
...
const DEFAULT_CONFIG: EngineConfig = {
  ...
  runtime: {
    ...
    skillBudget: 0.8,
    skillActivationMode: "activator",
    stickyTtlTurns: 8,
    stickyMaxSlots: 3,
    candidateTopK: 20,
    enableSearchSkillsTool: false,
    recallTopN: 10,
...
const asOptionalSystemPromptBase = (
  value: unknown,
  field: string,
): string | undefined => {
  if (value === undefined) {
  ...
  if (value.length === 0 || value.trim().length === 0) {
    throw ValidationError.invalidConfig(`${field} 不能为空字符串`);
  }
  const bytes = new TextEncoder().encode(value).length;
  if (bytes > SYSTEM_PROMPT_BASE_MAX_BYTES) {
    throw ValidationError.invalidConfig(
      `${field} 不能超过 ${SYSTEM_PROMPT_BASE_MAX_BYTES} 字节（当前 ${bytes}）`,
    );
  }
  return value;
};
...
export const validateEngineConfig = (raw: unknown): EngineConfig => {
  ...
  return {
    ...
    runtime: {
      ...
      skillBudget: asNumber(
        runtime.skillBudget,
        "runtime.skillBudget",
        DEFAULT_CONFIG.runtime.skillBudget ?? 0.8,
        { min: 0, max: 1 },
      ),
      skillActivationMode: (() => {
        const mode = runtime.skillActivationMode;
        if (mode === undefined) {
          return DEFAULT_CONFIG.runtime.skillActivationMode ?? "activator";
        }
        if (mode === "legacy") {
          throw ValidationError.invalidConfig(
            'runtime.skillActivationMode="legacy" 已退役；请改用 "activator"',
          );
        }
        if (mode !== "activator") {
          throw ValidationError.invalidConfig(
            `runtime.skillActivationMode 必须是 "activator"，收到: ${String(mode)}`,
          );
        }
        return mode;
      })(),
      stickyTtlTurns: asNumber(
        runtime.stickyTtlTurns,
        "runtime.stickyTtlTurns",
        DEFAULT_CONFIG.runtime.stickyTtlTurns ?? 8,
        { min: 1, max: 100 },
      ),
      stickyMaxSlots: asNumber(
        runtime.stickyMaxSlots,
        "runtime.stickyMaxSlots",
        DEFAULT_CONFIG.runtime.stickyMaxSlots ?? 3,
        { min: 1, max: 32 },
      ),
      candidateTopK: asNumber(
        runtime.candidateTopK,
        "runtime.candidateTopK",
        DEFAULT_CONFIG.runtime.candidateTopK ?? 20,
        { min: 1, max: 100 },
      ),
      enableSearchSkillsTool: asBoolean(
        runtime.enableSearchSkillsTool,
        "runtime.enableSearchSkillsTool",
        DEFAULT_CONFIG.runtime.enableSearchSkillsTool ?? false,
      ),
      recallTopN: asNumber(
        runtime.recallTopN,
        "runtime.recallTopN",
        DEFAULT_CONFIG.runtime.recallTopN ?? 10,
        { min: 1 },
      ),
      activationThreshold: asNumber(
        runtime.activationThreshold,
        "runtime.activationThreshold",
        DEFAULT_CONFIG.runtime.activationThreshold ?? 0.8,
        { min: 0, max: 1 },
      ),
      ...(runtime.maxTurnRetries !== undefined
        ? {
            maxTurnRetries: asNumber(
              runtime.maxTurnRetries,
              "runtime.maxTurnRetries",
              0,
              { min: 0, max: 16 },
            ),
 ...
 // toolUse 可选
    ...
    ...(raw !== null && typeof raw === "object" && !Array.isArray(raw) && "directAnswer" in raw && (raw as Record<string, unknown>).directAnswer !== undefined
      ...
      : {}),
    ...(raw !== null && typeof raw === "object" && !Array.isArray(raw) && "validation" in raw && (raw as Record<string, unknown>).validation !== undefined
      ? {
          validation: (() => {
            const validationRaw = asRecord(
              (raw as Record<string, unknown>).validation,
              "validation",
            );
            const result: NonNullable<EngineConfig["validation"]> = {};
            const semanticJudgeSystemPromptBase = asOptionalSystemPromptBase(
              validationRaw.semanticJudgeSystemPromptBase,
              "validation.semanticJudgeSystemPromptBase",
            );
            if (semanticJudgeSystemPromptBase !== undefined) {
              result.semanticJudgeSystemPromptBase = semanticJudgeSystemPromptBase;
            }
            return result;
          })(),
        }
