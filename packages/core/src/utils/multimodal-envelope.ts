...
/** @deprecated Use {@link buildIncludeToolsInputEnvelope} with host tool names. */
export function buildTextToImageInputEnvelope(
  prompt: string,
  source = "text-to-image-envelope",
  includeTools: readonly string[] = [],
): InputEnvelope {
  return buildIncludeToolsInputEnvelope(prompt, includeTools, source);
}
