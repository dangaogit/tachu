...
describe("QwenProviderAdapter", () => {
 ...
 describe("embed (fixture coverage)", () => {
    interface InnerEmbedCall {
      request: EmbeddingRequest;
      signalProvided: boolean;
    }

    const stubInner = (
      adapter: QwenProviderAdapter,
      handler: (call: InnerEmbedCall) => EmbeddingResponse,
    ): InnerEmbedCall[] => {
      const calls: InnerEmbedCall[] = [];
      const fakeInner = {
        async embed(request: EmbeddingRequest, _ctx: unknown, signal?: AbortSignal) {
          const call = { request, signalProvided: signal !== undefined };
          calls.push(call);
          return handler(call);
        },
      };
      Reflect.set(adapter, "inner", fakeInner);
      return calls;
    };

    it("delegates exact model + string input list + outputDimensionality to inner OpenAI-compatible client", async () => {
      const adapter = new QwenProviderAdapter({ apiKey: "k" });
      const calls = stubInner(adapter, (call) => ({
        embeddings: call.request.inputs.map(() => [0.1, 0.2, 0.3]),
      ...
      expect(calls[0]!.request.model).toBe("text-embedding-v4");
      expect(calls[0]!.request.inputs).toEqual(["你好", "world"]);
      expect(calls[0]!.request.outputDimensionality).toBe(256);
      expect(result.embeddings).toHaveLength(2);
      expect(result.usage.promptTokens).toBe(3);
    });

    it("propagates AbortSignal to inner.embed when caller passes one", async () => {
      const adapter = new QwenProviderAdapter({ apiKey: "k" });
      const calls = stubInner(adapter, () => ({
        embeddings: [[0.1]],
    ...
    it("forwards mixed text-part inputs verbatim (no flattening on the Qwen wrapper)", async () => {
      const adapter = new QwenProviderAdapter({ apiKey: "k" });
      const calls = stubInner(adapter, (call) => ({
        embeddings: call.request.inputs.map(() => [0.4, 0.5]),
        usage: { promptTokens: 2, completionTokens: 0, totalTokens: 2 },
      }));

      const multimodalInputs: EmbeddingRequest["inputs"] = [
        "single string",
        [
      ...
      expect(calls[0]!.request.inputs).toEqual(multimodalInputs);
      expect(calls[0]!.request.inputs).toHaveLength(2);
    });

    it("omits outputDimensionality when caller does not specify it", async () => {
      const adapter = new QwenProviderAdapter({ apiKey: "k" });
      const calls = stubInner(adapter, () => ({
        embeddings: [[0.9]],
      ...
      await adapter.embed(
        { model: "text-embedding-v4", inputs: ["x"] } satisfies EmbeddingRequest,
        DEFAULT_ADAPTER_CALL_CONTEXT,
      );

      expect(calls[0]!.request.outputDimensionality).toBeUndefined();
    });
  });
