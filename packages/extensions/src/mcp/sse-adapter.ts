...
const createRequestId = (): string =>
  `mcp-sse-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;

/**
 * MCP SSE 传输适配器。
 */
export class McpSseAdapter implements McpToolAdapter {
  readonly transport = "sse" as const;

  private readonly options: McpSseAdapterOptions;
  private readonly maxReconnectAttempts = 3;
  private readonly defaultTimeoutMs: number;
  private client: Client | null = null;
  private transportImpl: SSEClientTransport | null = null;
 ...
 */
 ...
 // ：为 callTool 套上 adapter 层默认超时，避免远端挂起拖垮引擎。
    ...
    if (this.defaultTimeoutMs > 0) {
      timeoutHandle = setTimeout(() => {
        ...
        controller.abort(
          new Error(
            `MCP sse request timed out after ${this.defaultTimeoutMs}ms: ${mappedRequestId}`,
          ),
        );
      }, this.defaultTimeoutMs);
    }
