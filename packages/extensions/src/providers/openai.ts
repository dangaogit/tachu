...
const mapMessage = (message: Message): Record<string, unknown> => {
 ...
 // Agentic Loop 历史时，assistant 消息可能伴随结构化 tool_calls。
...
const embeddingInputToString = (input: EmbeddingContent): string => {
  ...
  async embed(
    ...
    try {
      const response = await this.client.embeddings.create(
        {
          model: request.model,
          input: request.inputs.map(embeddingInputToString),
