...
- 协议、类型、配置与错误码以本 README 与源码类型定义为准（见 §4–§8）。
...
下列变量与 `src/config/load-config.ts` 及 `src/types/config.ts` 一致；默认值与校验范围以源码为准。
...
| `/v1/search` | POST | 搜索 + 可选批量 extract；`web-search` 主路径（stub 时返回 503） |

详细 JSON schema、鉴权与大小限制见 `src/routes/extract.ts`、`src/routes/search.ts` 及同目录类型定义。
...
镜像构建与多阶段说明见同目录 `Dockerfile` 与 `docker-compose.yml`。Dockerfile 的 Playwright image 与 Chromium revision 由 `scripts/validate-release-artifacts.ts` 校验，需与根 catalog 中的 `playwright-core` 保持一致。
...
2. **`403 SSRF_BLOCKED` / 无法访问目标 URL**  
   目标落在私网、元数据地址或被 SSRF 规则拦截属预期。仅抓取公网 URL；必要时配置 `WEB_FETCH_ALLOWED_DOMAINS` / `WEB_FETCH_BLOCKED_DOMAINS`（逗号分隔域名，见 §4）。
...
摘自 `src/errors/unifier.ts`；完整 `detail` 与处置见路由层错误映射实现。
...
- **web-search**：与 web-fetch **共用同一 Server 基址与 token**，主调 `POST /v1/search`；未配置真实 provider（`WEB_SEARCH_PROVIDER=stub`）时返回 503；配置 `brave` / `tavily` / `searxng` 后由服务端转发至对应 API。请求/响应 schema 见 `src/routes/search.ts` 与 `src/search/provider.ts`。
