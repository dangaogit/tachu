...
# @see .cursor/web-fetch-workflow/contracts/-server-smoke-check.md
...
# WEB_FETCH_ALLOW_LOOPBACK：否则对 127.0.0.1 的上游 URL 会被 SSRF 拒绝。
WEB_FETCH_PORT=8787 \
...
# 成功响应字段为 `body`（契约 §3 文案写为 content，此处按 ADR 校验）。
echo "${RESP}" | bun -e '
